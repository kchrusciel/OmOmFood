...
class OfferControllerSpec extends Specification {
    ...
    def "Should return offer by ID when get '/offers/{id}'" () {
        given:
            def offerFromDb = OfferDto.builder()
                    .id(1)
                    .authorId(1)
                    ...
                    .content("content")
                    .build()
            when(facade.findById(1)).thenReturn(offerFromDb)
        when:
            def result = offer.perform(get("/offers/1"))
        then:
            result.andExpect(content().json(mapper.writeValueAsString(offerFromDb)))
                    .andExpect(status().isOk())
    }
